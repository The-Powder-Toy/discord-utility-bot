...
for _, row in pairs(from_handle:exec(log:sub("setting up source db"), [[SELECT * FROM connections]])) do
	to_handle:exec(log:sub("connecting duser $ with tuser $ aka $", row.duser, row.tuser, row.tname), [[INSERT INTO connections ("duser", "tuser", "tname", "stale") values (?, ?, ?, 0)]], row.duser, row.tuser, row.tname)
end
